...
async def accept_order(
    ...
    try:
        ...
        text = (
            ...
            f"<i>📍 Поделитесь с заказчиком транслируемой геопозицией, чтобы он мог видеть текущее местоположение заказа.</i>\n"
            f"<i>Нажмите 📎 → Геопозиция → Транслировать геопозицию.</i>\n\n"
            f"<i>✅ После завершения заказа перейдите в:</i>\n"
            f"<i>Меню → Мои заказы → Активные</i>\n"
            f"<i>и нажмите кнопку '✅ Выполнил'</i>\n\n"
            f"🔗 <b>Нажмите для перехода в список заказов →</b> /my_orders\n"
        )
