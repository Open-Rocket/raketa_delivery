...
class AssistantOpenAi:
    ...
    async def process_order(
        ...
        moderation = """
            ...
            - Если сообщение — это обычный заказ доставки (в пределах города, пешком), и оно соответствует контексту сервиса, тогда обрабатывай.
            """
        # moderation = "Пропускай все!"
        ...
        parsed_address = (
            ...
            "(Город, улица, дом, корпус, если доступно). Например: 'Город, адрес, улица/дом/корпус/номер/подъезд'. "
            "Если указан подъезд то добавляй его в адрес. "
            "Убедись, что все элементы адреса извлечены корректно и без ошибок."
            "Если адресов нет то верни 'no_address'"
            "Уточнения вроде домофона, этажа и т.д. не учитывай, только адреса без лишних деталей. "
            "Все уточнения переноси в описание заказа, такие как пожелания номера квартир и домофоны."
            "Старайся структурно но как человек делать описания. "
        )
        description = "Опиши грамотно и коротко заказ. Детали если они есть и комментарии, но без адресов. Если чего то не говорится не пиши, ты делаешь описание для курьера. Тезисно."
