...
async def osrm():
    ...
    distance_m, duration_m = await calculate_total_distance(
        coords_list[3], adjustment_factor=1.32
    )
    distance_m2, duration_m2 = await calculate_manhattan_distance(
        coords_list[3], adjustment_factor=1.1
    )

    print(
        f"test osrm - Общая дистанция: {distance:.2f} км, Общее время: {duration} минут"
    )
    print(
        f"test adjustment - Общая дистанция: {distance_m:.2f} км, Общее время: {duration_m} минут"
    )
    print(
        f"test manhattan - Общая дистанция: {distance_m2:.2f} км, Общее время: {duration_m2} минут"
    )
