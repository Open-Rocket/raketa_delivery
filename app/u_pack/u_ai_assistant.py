...
async def process_order_text(order_text: str) -> dict:
    ...
    prompt = (
        ...
        f"  'Destination point C': 'Третий пункт доставки (если есть)',\n"
        f"  'Destination point D': 'Четвертый пункт доставки (если есть)',\n"
...
async def get_parsed_addresses(order_text):
    prompt = (f"Пожалуйста, извлеки из этого текста все возможные адреса в следующем формате:\n"
              f"(Тип объекта: улица, поселок, деревня, село, аэропорт, вокзал, станция, станция метро и т.д., "
