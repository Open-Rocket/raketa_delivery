...
async def get_parsed_addresses(order_text, city=None):
    ...
    if not city:
        prompt = (f"Пожалуйста, извлеки из этого текста все адреса в формате:\n"
                  f"(Город Тип объекта Название объекта Номер здания).\n"
                  f"Пример: Москва проспект Вернадского 76/2.\n"
                  f"addresses: [(Перечисли адреса в указанном формате)]\n"
                  f"text: {order_text}. "
                  f"Если второй город не указан, считай, что оба адреса из одного города.")
    else:
        prompt = (f"Пожалуйста, извлеки из этого текста все адреса в формате:\n"
                  f"(Город Тип объекта Название объекта Номер здания).\n"
                  f"Пример: Москва проспект Вернадского 76/2.\n"
    ...
    # Регулярное выражение для извлечения корректных адресов
    address_pattern = re.compile(
        r'([А-Яа-яЁё]+)\s(проспект|улица|площадь|квартал|мкр|тупик|пер\.)\s([А-Яа-яЁё\s]+(?:\d{1,4}(?:\/\d+)?)?)')

    # Находим все подходящие адреса
    matches = address_pattern.findall(response)

    # Формируем корректные адреса
    addresses = [f'{match[0]} {match[1]} {match[2].strip()}' for match in matches]

    # Если адреса не были найдены, пробуем другие способы или выводим ошибку
    if not addresses:
        print("Не удалось извлечь адреса из текста.")
        addresses = response.splitlines()  # Разбиваем текст на строки, возможно, это поможет

    # Убираем лишние слова и проверяем на корректность
    cleaned_addresses = []
    for address in addresses:
        # Убираем избыточные слова
        cleaned_address = re.sub(r'\s*(дом|корпус|угол|кв|пом|строение|павильон)\s*', ' ', address)
        # Проверяем, чтобы не было сокращений типа "Мос а"
        cleaned_address = cleaned_address.replace("Мос а", "Москва")
        cleaned_addresses.append(cleaned_address.strip())

    return cleaned_addresses  # f"Город (обязательно),Тип объекта: улица, поселок, деревня, село, аэропорт, "\


#                   f"вокзал, станция, станция метро и т.д., "
...
#                   f"Город (обязательно),Тип объекта: улица, поселок, деревня, село, аэропорт, "
#                   f"вокзал, станция, станция метро и т.д., "
#                   f"Название объекта, номер улицы или дома, город или населенный пункт).\n"
#                   f"addresses: [(Передай в этот список кортеж из адресов по указанному формату)]\n"
