...
async def assistant_censure(req: str) -> str:
    ...
    instructions = {
        "censure": (
            ...
            f"{censore_data[0]} — если текст полностью чист, "
            f"и не содержит запрещенных товаров, и подозрительности. "
            f"(и  является заказом с пунктами доставки, это важно),"
            f"(и предмет доставки указывается, это тоже важно будь внимателен).\n"
            f"{censore_data[1]} — если текст содержит табачные изделия, электронные сигареты, алкоголь."
            f"(и  является заказом с пунктами доставки, это важно).\n"
            f"{censore_data[2]} — если текст  неразборчивый, неполный или это билиберда.\n"
            f"{censore_data[3]} — если в тексте  не говорится о том что нужно доставить. "
            f"(и текст является заказом с пунктами доставки, это важно).\n"
            f"{censore_data[4]} — если текст содержит запрещенные товары (наркотики, оружие, животные и прочие).\n"
            f"или подозрительный текст, указывающий на незаконную деятельность. "
            f"(и текст является заказом с пунктами доставки, это важно).\n"
            f"{censore_data[5]} - если текст вообще не является заказом доставки или эта доставка не реалистичная. "
            f"(например из мира фентези, научной фантастики, просто фантастики или неосуществим)."
            f"Заказ: {req}"
        )
    }
