...
#         await handle_existing_user(state, handler, message)
#     else:
#         await handle_new_user(state, handler, message)


# async def handle_existing_user(state: FSMContext, handler: MessageHandler, message: Message):
...
# async def handle_new_user(state: FSMContext, handler: MessageHandler, message: Message):
...
#     name = message.text
#     if len(name) > 42:
#         await handle_long_name(handler, message)
#     else:
#         await handle_valid_name(state, handler, message, tg_id, name)


# async def handle_long_name(handler: MessageHandler, message: Message):
...
# async def handle_valid_name(state: FSMContext, handler: MessageHandler, message: Message, tg_id: int, name: str):
...
#         "в случае необходимости, пожалуйста, нажмите на кнопку 'Поделиться номером'!\n\n"
#         "<i>*При регистрации с компьютера нажмите на значек команд рядом с полем ввода.</i>\n\n"
#         "<b>Ваш номер:</b>"
#     )
...
#     text = (
#         "Последний шаг!\n\n"
#         "Для того чтобы каждый раз не указывать город доставки, "
#         "скажите в каком городе вы будете в основном делать заказы "
#         "и он автоматически будет подставляться.\n\n"
#         "<b>Ваш город:</b>"
#     )
#     msg = await message.answer(text, disable_notification=True, parse_mode="HTML")
#     await handler.handle_new_message(msg, message)
...
#     text = (
#         "Начиная использование сервиса, вы соглашаетесь с "
#         "<a href='https://drive.google.com/file/d/1iKhjWckZhn54aYWjDFLQXL46W6J0NhhC/view?usp=sharing'>"
#         "Пользовательским соглашением и правилами использования</a>, а также "
#         "<a href='https://telegram.org/privacy'>Политикой конфиденциальности</a>.\n\n"
#         "<i>*Обращаем внимание, что любые действия, связанные с заказами, "
#         "отправкой или получением посылок, должны соответствовать законодательству "
#         "вашего государства и общепринятым этическим нормам.</i>\n\n"
#     )
...
#     text = (
#         "👥 <b>Профиль</b>\n\n"
#         "Посмотрите или измените данные о себе.\n\n"
#         "• Номер нужен для связи с курьером.\n"
#         "• Город подставляется в заказ.\n\n"
#         "<i>*При заказе в другом городе укажите его в описании к заказу.</i>\n\n"
#         f"<b>Имя:</b> {name} \n"
...
#     recognized_text = await recognize_message_content(message)
#     if not recognized_text:
#         await handle_recognition_failure(handler, message, wait_message)
#         return
...
#         await handle_clear_order(state, handler, message, recognized_text, wait_message)
#     else:
#         await handle_censorship_failure(state, handler, message, most_compatible_response, wait_message)


# async def recognize_message_content(message: Message) -> str:
...
# async def handle_recognition_failure(handler: MessageHandler, message: Message, wait_message: Message):
#     reply_kb = await get_user_kb(text="voice_order_accept")
#     new_message = await message.answer("Ошибка распознавания. Попробуйте снова.", reply_markup=reply_kb)
#     await wait_message.delete()
#     await handler.handle_new_message(new_message, message)


# async def handle_clear_order(state: FSMContext, handler: MessageHandler, message: Message, recognized_text: str, wait_message: Message):
#     user_city = await user_data.get_user_city(message.from_user.id)
#     addresses = await get_parsed_addresses(recognized_text, user_city)
#     if len(addresses) == 2:
#         await process_two_addresses_order(state, handler, message, recognized_text, addresses)
#     elif len(addresses) == 3:
#         await process_three_addresses_order(state, handler, message, recognized_text, addresses)
#     elif len(addresses) == 4:
#         await process_four_addresses_order(state, handler, message, recognized_text, addresses)
#     elif len(addresses) == 5:
#         await process_five_addresses_order(state, handler, message, recognized_text, addresses)
#     else:
#         await handle_invalid_order(handler, message, recognized_text)
#     await wait_message.delete()


# async def handle_censorship_failure(state: FSMContext, handler: MessageHandler, message: Message, response: str, wait_message: Message):
#     await state.set_state(UserState.default)
#     reply_kb = await get_user_kb(text="rerecord")
#     if response == "inaudible":
#         text = "<b>Ошибка</b> ⸘\n\nТекст сообщения неразборчив.\nПопробуйте отправить заказ снова."
#     elif response == "no_item":
#         text = "<b>Что везем?!</b> \n\nКурьер должен знать что он доставляет."
#     elif response == "not_order":
#         text = "<b>...</b> 🫤 \n\nСделайте корректный заказ!"
#     elif response == "intercity":
