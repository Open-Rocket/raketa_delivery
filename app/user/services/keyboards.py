...
async def get_my_orders_kb(
    ...
    my_orders_kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=f"Отмененные {canceled_count}", callback_data="canceled_orders"
                ),
                InlineKeyboardButton(
...
async def get_poll():
    poll_button = ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(
                    text="Создать опрос", request_poll=KeyboardButtonPollType()
                )
            ]
        ],
        resize_keyboard=True,
    ...
    switch_button = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Поделиться",
                    switch_inline_query="👈 Лучший сервис пешей доставки!",
                )
            ]
        ]
    )
